...
We retain the simplified parametrization of equation~\refp{eq:specia} as
accounting for distributions in metallicity would require folding in some sort
of galactic chemical evolution model (see, e.g.,~\S~\ref{sec:gce} below or the
review by~\citealp{Matteucci2021}).
}
...
over the SMF).
{\color{red}
However, the~\um~SFHs and its predicted SMF are closely related as well.
}
\par
{\color{red}
...
To add to the complication, other MZRs in the literature find stronger
differences in the shape than~\citet{Andrews2013} and~\citet{Zahid2014} at the
low mass end~\citep[see, e.g., the review by][]{Kewley2019}.
